...
public class NavigationViewModel extends AbstractViewModel {
    ...
    public void navigateForward(@NonNull final String parentNodeId) {
        ...
        if (this.currentParentNodeId.isPresent()) {
            // TODO: Check optional
            this.history.push(ImmutableHistoryItem.of(this.currentParentNodeId.get(),
    ...
    /**
     * Creates a navigation state from a list of nodes. <em>Warning:</em> This only works correctly
     * if one request is processed at a time.
     *
     * @param nodes
     *            The {@link List} of {@link Node}s.
     * @return A {@link NavigationState} containing the nodes.
     */
    private NavigationState createNavigationState(List<Node> nodes) {
        return ImmutableNavigationState.builder().withParentNodeId(this.currentParentNodeId.get())
                .withNodes(nodes).withScrollPosition(this.currentScrollPosition).build();
    }
